...
func _ready() -> void:
	$Camera/Player.start($Level_TEST/StartPosition.position)


func _on_player_shoot() -> void:
	if(playerBulletsOnScreen < maxPlayerBullets):
		playerBulletsOnScreen += 1 
		var scene_instance = test_bullet.player_new_bullet($Camera/Player.get_bullet_spawn_point(), Vector2.RIGHT)
